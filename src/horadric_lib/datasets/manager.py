...
# These can eventually be moved to your config file
DATA_DIR = Path("data")
...
def fetch_huggingface_dataset(dataset_id: str, output_path: Path):
...
def load_raw_dataset(
    source: str, force_download: bool = False, target_filename: Optional[str] = None
) -> pd.DataFrame:
    """
    ...
    if age_days > SIX_MONTHS_DAYS:
        choice = input(
            f"Cache {filename_to_check} is > 6 months old. Redownload? [y/N]: "
        ).lower()
        if choice == "y":
            filepath = get_fresh_remote_dataset(source, registry)
...
def create_dataset_slice(
    ...
    if sort_col and sort_col in df.columns:
        ...
        df_slice = df.sort_values(sort_col, ascending=ascending).head(
            int(len(df) * top_p)
        )
    else:
        if top_p < 1.0:
    ...
    if test_size > 0.0:
        ...
        train_df, test_df = train_test_split(
            df_slice, test_size=test_size, random_state=random_seed
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--data-dir", type=str, default=conf.data_dir, help="Path to the data"
    )
    parser.add_argument(
        "--dataset-name",
        type=str,
    ...
    if args.dataset_name == "SATBench":
        raw_df = raw_df.rename(
            columns={"readable": "original_cnf", "scenario": "scenario_text"}
        )
